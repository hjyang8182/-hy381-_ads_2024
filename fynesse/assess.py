...
def find_avg_lsoa_price_in_lad(conn, lad_id, lad_boundaries): 
    ...
    cur = conn.cursor(pymysql.cursors.DictCursor)
    cur.execute(f"select lsoa_id, avg(price) as avg_price from prices_coordinates_oa_data where latitude between {south} and {north} and longitude between {west} and {east} and date_of_transfer >= '2020-01-01' group by lsoa_id limit 15")
